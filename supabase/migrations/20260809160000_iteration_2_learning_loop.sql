...
create function public.can_read_assessment_score(p_attempt uuid) returns boolean language sql stable security definer set search_path='' as $$ select exists(select 1 from public.assessment_attempts a join public.assignments x on x.id=a.assignment_id join public.classes c on c.id=x.class_id where a.id=p_attempt and(a.learner_id=auth.uid()or c.teacher_id=auth.uid()or public.has_guardian_access(a.organization_id,a.learner_id))) $$;
revoke all on function public.can_read_assessment_score(uuid)from public;grant execute on function public.can_read_assessment_score(uuid)to authenticated;
...
create policy score_authorized_read on public.assessment_scores for select using(public.can_read_assessment_score(attempt_id));
