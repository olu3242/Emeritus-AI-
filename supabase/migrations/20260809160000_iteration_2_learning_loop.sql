...
create policy score_authorized_read on public.assessment_scores for select using(exists(select 1 from public.assessment_attempts a join public.assignments x on x.id=a.assignment_id join public.classes c on c.id=x.class_id where a.id=attempt_id and(a.learner_id=auth.uid()or c.teacher_id=auth.uid()or exists(select 1 from public.guardian_relationships g where g.organization_id=a.organization_id and g.guardian_id=auth.uid()and g.learner_id=a.learner_id and g.status='active'))));
create policy mastery_authorized_read on public.mastery_decisions for select using(learner_id=auth.uid()or exists(select 1 from public.guardian_relationships g where g.organization_id=mastery_decisions.organization_id and g.guardian_id=auth.uid()and g.learner_id=mastery_decisions.learner_id and g.status='active')or public.has_org_role(organization_id,array['educator','administrator','platform_administrator']::public.app_role[]));
create policy recommendation_authorized_read on public.learning_recommendations for select using(learner_id=auth.uid()or exists(select 1 from public.guardian_relationships g where g.organization_id=learning_recommendations.organization_id and g.guardian_id=auth.uid()and g.learner_id=learning_recommendations.learner_id and g.status='active')or public.has_org_role(organization_id,array['educator','administrator','platform_administrator']::public.app_role[]));
...
with scored as(select i.id,i.points,case when r->>'option'=i.answer_key->>'correctOption'then i.points else 0 end got from public.assessment_items i left join lateral(select value r from jsonb_array_elements(p_responses)where value->>'itemId'=i.id::text limit 1)x on true where i.assessment_version_id=a.assessment_version_id),agg as(select coalesce(sum(got),0)e,coalesce(sum(points),0)p,jsonb_agg(jsonb_build_object('itemId',id,'earned',got,'possible',points))o from scored)select e,p,o into earned,possible,outcomes from agg;
