...
declare a public.assessment_attempts;c public.classes;rv public.rubric_versions;revision public.human_score_revisions;objective uuid;decision public.mastery_decisions;ratio numeric;next_state public.mastery_state;kind text;
...
if exists(select 1 from public.human_score_revisions where organization_id=a.organization_id and idempotency_key=p_idempotency)then return jsonb_build_object('replayed',true);end if;if coalesce((select max(revision)from public.human_score_revisions where attempt_id=a.id),0)<>p_expected_revision then raise exception'concurrent grading revision'using errcode='40001';end if;
insert into public.human_score_revisions(organization_id,attempt_id,rubric_version_id,revision,grader_id,criterion_scores,earned,possible,rationale,status,idempotency_key,finalized_at)values(a.organization_id,a.id,rv.id,p_expected_revision+1,auth.uid(),p_criterion_scores,p_earned,p_possible,p_rationale,'finalized',p_idempotency,now())returning*into revision;
insert into public.assessment_scores(organization_id,attempt_id,scoring_version,earned,possible,outcomes)values(a.organization_id,a.id,'human:'||rv.version,p_earned,p_possible,jsonb_build_object('revision',revision.revision,'rubricVersionId',rv.id));update public.assessment_attempts set state='scored',finalized_at=now()where id=a.id;
